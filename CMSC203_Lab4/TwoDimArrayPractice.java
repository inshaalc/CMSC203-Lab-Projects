...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void printArray( )
   {
	   ...
	   for (int row = 0; row < intArray.length; row++)
	   {
		   // creates array of rows, inherited from the intArray[row]
		   int [] rows = intArray[row];
		   System.out.println(" ");
		   System.out.print(row + "\t");
		   
		   // inner loop goes column by column in each row, and prints each value in each column of each row 
   ...
   */
   public void setValues( int value, int row )
   {
    ...
    // Write your code here:
	   ...
	   // for loop simply sets all elements in specified row to specified value (in each column in that row) 
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	  ...
	  // min starts from the beginning of the array
	  int min = intArray[0][column];
	  
	  // for loop goes through each row in the array 
	  for (int row = 0; row < intArray.length; row++)
	  {
		  // updates minimum if the current specified point in the array is less than the previous minimum
   ...
   */
   public int countFound( int value )
   {
    ...
    // Write your code here:
	   ...
	   {
	       int[] row = intArray[rows];
	       
	       // inner loop
	       for (int col = 0; col < row.length; col++) 
	       {
	    	   // if statement checks to see if value is found in array
	           if (row[col] == value) 
	           {
	               num++;
	           }
	           animate(rows, col, value);
	       }
	   }
	   return num;
   }
